...
TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "ai_flow_footer":        {"en": "All AI features activate with a single config: <code>ai_provider</code>. Supports 11 LLM providers including DeepSeek, Claude, Gemini, and OpenAI-compatible.", "zh": "所有 AI 功能只需一项配置：<code>ai_provider</code>。支持 11 家 LLM 供应商，包括 DeepSeek、Claude、Gemini 及 OpenAI 兼容接口。"},
    ...
    "cl_2026_09_win_polish": {
        "en": "<strong>Windows desktop polish</strong>: startup window-size jump fixed — on RDP the display DPI arrives seconds after process start, so the frame stays hidden until its DPI settles (validated against the registry <code>AppliedDPI</code>), is sized in physical pixels, then shown once. Unified <code>Segoe UI</code> 10pt typography across every child window (wx never propagates font changes), bold grid headers, and a 14pt monospace log console with theme colours preserved. Toolbar &amp; custom controls restyled: native 3D buttons flattened via a central <code>PETPTheme</code> palette with hover states and hairline separators at uniform 28-DIP heights; <code>PopupMenuButton</code> switchers match sibling buttons; <code>ToggleSwitch</code> redrawn Win11-Fluent; <code>RunButton</code> / <code>ThemedButton</code> / <code>ToggleSwitch</code> gained <code>wx.BORDER_NONE</code>. Grid fixes: 34-DIP task / execution rows so two-line renderer text never clips, a new <code>ProcessorNameRenderer</code> that ellipsizes column 0 with a ⊖ skipped marker, and <code>McpDescEditor</code> grids with font-aware row heights plus a rewritten column fit — a narrow-panel shrink no longer sticks and headers never truncate.",
        "zh": "<strong>Windows 桌面观感打磨</strong>：修复启动时窗口尺寸跳动 —— RDP 下显示器 DPI 在进程启动数秒后才就绪，现在窗口保持隐藏直至 DPI 稳定（对照注册表 <code>AppliedDPI</code> 校验），按物理像素设定尺寸后一次性显示。统一 <code>Segoe UI</code> 10pt 排版（wx 不会向已创建的子窗口传播字体变更），加粗表格表头，14pt 等宽日志控制台且保留主题配色。工具栏与自定义控件重塑：原生 3D 按钮经由中央 <code>PETPTheme</code> 调色板扁平化，带悬停态与发丝级分隔线，统一 28-DIP 高度；<code>PopupMenuButton</code> 切换器与相邻按钮对齐；<code>ToggleSwitch</code> 重绘为 Win11-Fluent 风格；<code>RunButton</code> / <code>ThemedButton</code> / <code>ToggleSwitch</code> 加上 <code>wx.BORDER_NONE</code> 去掉默认系统边框。表格修复：任务 / 执行表格行高 34 DIP，两行文本不再裁剪；新增 <code>ProcessorNameRenderer</code> 省略号截断并带 ⊖ 跳过标记；<code>McpDescEditor</code> 表格按字体自适应行高并重写列宽拟合 —— 窄面板缩回不再卡死，表头任何宽度下都不截断。",
    },
    "cl_2026_09_v150": {
        "en": "<strong>Windows GUI overhaul + safe YAML + file-IO fixes (V1.5.0)</strong>: refreshed Windows look &amp; feel — theme-aware widgets, tighter layout, and native rendering polish across the task editor. Security: execution/pipeline YAML is loaded via <code>PETPSafeLoader</code> (built on <code>SafeLoader</code>) which registers constructors for <strong>only</strong> the four project classes (<code>Execution</code>, <code>Task</code>, <code>Pipeline</code>, <code>Loop</code>) — <code>yaml.Loader</code>/<code>UnsafeLoader</code>/<code>FullLoader</code> are never used, so <code>!!python/object/apply:...</code> tags cannot execute arbitrary code at load time; any unregistered tag raises <code>ConstructorError</code>. Fixes: <code>READ_EXCEL</code> resolves <code>file_path</code> expressions correctly, and <code>WRITE_TO_FILE</code> gains an append <code>mode</code>.",
        "zh": "<strong>Windows GUI 全面增强 + 安全 YAML + 文件 IO 修复（V1.5.0）</strong>：重做 Windows 观感——主题感知控件、更紧凑的布局与任务编辑器的原生渲染优化。安全：执行/流水线 YAML 经由 <code>PETPSafeLoader</code>（基于 <code>SafeLoader</code>）加载，<strong>仅</strong>为四个项目类（<code>Execution</code>、<code>Task</code>、<code>Pipeline</code>、<code>Loop</code>）注册构造器——从不使用 <code>yaml.Loader</code>/<code>UnsafeLoader</code>/<code>FullLoader</code>，因此 <code>!!python/object/apply:...</code> 标签无法在加载时执行任意代码；任何未注册标签都会抛出 <code>ConstructorError</code>。修复：<code>READ_EXCEL</code> 正确解析 <code>file_path</code> 表达式，<code>WRITE_TO_FILE</code> 新增追加 <code>mode</code>。",
    },
    "cl_2026_07_ariba_forms": {
        "en": "<strong>SAP Ariba Angular form automation + headless resilience</strong>: new Selenium processors that drive SAP Ariba's Angular Material forms end-to-end — <code>SELECT_YESNO</code> (是/否 radio by visible text), <code>SELECT_TREE_DROPDOWN</code> (cascading tree picker), <code>SELECT_MULTI_DROPDOWN</code> (atomic <code>execute_script</code> to defeat the virtual-scroller), <code>SELECT_SINGLE_DROPDOWN</code> (type-ahead), and <code>DATE_PICKER</code> (Angular Material calendar matched by locale-independent English <code>aria-label</code>), plus bank / contact form feeders and a file-upload processor. Diagnostics: <code>timeout_msg</code> appends a business message (with <code>{supplier_name}</code> / <code>{timeout}</code> expressions) to locator-timeout exceptions, and <code>[NOOP]</code> logging makes silent \"green but did nothing\" soft-skips greppable. <code>GO_TO_PAGE</code> gains a per-Execution <code>browser_lang</code> so headless Chrome renders locale-sensitive sites in the right language. Shared <code>click_with_fallback</code> (native→move→JS) and <code>xpath_literal</code> helpers replace copy-pasted code.",
        "zh": "<strong>SAP Ariba Angular 表单自动化 + 无头稳健性</strong>:新增一批 Selenium 处理器,端到端驱动 SAP Ariba 的 Angular Material 表单 —— <code>SELECT_YESNO</code>(按可见文本选 是/否 单选)、<code>SELECT_TREE_DROPDOWN</code>(级联树形选择)、<code>SELECT_MULTI_DROPDOWN</code>(单次原子 <code>execute_script</code> 攻克虚拟滚动)、<code>SELECT_SINGLE_DROPDOWN</code>(联想输入)、<code>DATE_PICKER</code>(Angular Material 日历,按与语言无关的英文 <code>aria-label</code> 定位),外加银行/联系人表单填充器与文件上传处理器。诊断能力:<code>timeout_msg</code> 在定位超时抛异常时附带业务信息(支持 <code>{supplier_name}</code> / <code>{timeout}</code> 表达式),<code>[NOOP]</code> 日志让\"看着绿但啥也没干\"的软跳过可被 grep。<code>GO_TO_PAGE</code> 新增按 Execution 传入的 <code>browser_lang</code>,让无头 Chrome 以正确语言渲染 locale 敏感站点。共享的 <code>click_with_fallback</code>(native→move→JS)与 <code>xpath_literal</code> 助手取代各处复制粘贴的代码。",
    },
    ...
    "cl_2026_05_clean_code_refactor": {
        "en": "<strong>Refactor (clean code)</strong>: <code>PETPPresenter</code> shrunk by 128 lines — Snapshot/Undo/Redo state and 10 methods extracted into <code>mvp/presenter/sub/SnapshotManager.py</code>; thin wrappers preserved on the Presenter so existing call sites are unchanged. New <code>httpservice/HttpServerBaseMixin.py</code> shares 4 stateless handlers (<code>_require_token</code>, <code>_handle_health</code>, <code>_handle_mcp_get</code>, <code>_status_code_for_result</code>) between <code>HttpServer</code> and <code>BackgroundHttpServer</code>, eliminating the GUI/BG twin duplication. Behavior-preserving — all tests green.",
        "zh": "<strong>代码重构(clean code)</strong>:<code>PETPPresenter</code> 减少 128 行 — Snapshot/Undo/Redo 状态与 10 个方法抽到 <code>mvp/presenter/sub/SnapshotManager.py</code>;Presenter 端保留 thin wrapper,现有调用点零变化。新增 <code>httpservice/HttpServerBaseMixin.py</code>,把 <code>_require_token</code>、<code>_handle_health</code>、<code>_handle_mcp_get</code>、<code>_status_code_for_result</code> 4 个无状态 handler 在 <code>HttpServer</code> 和 <code>BackgroundHttpServer</code> 之间共享,消除 GUI/BG 双胞胎重复。行为完全保留,全部测试通过。",
    },
    "cl_2026_05_security_phase2": {
        "en": "<strong>Security hardening (Phase 2)</strong>: <code>CMD</code> defaults to <code>shlex.split</code> (no shell); <code>_fn</code>/<code>lambda_*</code> sandbox drops <code>__import__</code>/<code>open</code>/<code>eval</code>/<code>exec</code>/<code>compile</code>/<code>getattr</code>/<code>hasattr</code>; encrypted-password salt externalized via <code>PETP_SALT</code> or <code>~/.petp/secret</code>; opt-in path traversal guard (<code>PETP_PATH_ALLOW_ROOTS</code>) for all file IO processors; HTTP body capped at 4 MiB &amp; JSON-RPC batch capped at 64 items; sensitive log values (<code>api_key</code>, <code>password</code>, <code>token</code>, <code>authorization</code>) redacted by default.",
        "zh": "<strong>安全加固（Phase 2）</strong>：<code>CMD</code> 默认走 <code>shlex.split</code>（不进 shell）；<code>_fn</code>/<code>lambda_*</code> 沙箱移除 <code>__import__</code>/<code>open</code>/<code>eval</code>/<code>exec</code>/<code>compile</code>/<code>getattr</code>/<code>hasattr</code>；加密密码 salt 通过 <code>PETP_SALT</code> 或 <code>~/.petp/secret</code> 外置；可选路径遍历守卫（<code>PETP_PATH_ALLOW_ROOTS</code>）覆盖所有文件 IO 处理器；HTTP body 限制 4 MiB，JSON-RPC 批量限制 64 项；日志中敏感字段（<code>api_key</code>、<code>password</code>、<code>token</code>、<code>authorization</code>）默认脱敏。",
    },
    "cl_2026_05_mcp_perf": {
    ...
    "cl_2026_05_ai_llm_unify": {
        "en": "Unified AI LLM processors: 11 providers (DeepSeek, Zhipu, Qianfan, MiniMax, Anthropic, Hyperspace, Doubao, Moonshot, Gemini, Ollama, OpenAI-compatible) with MCP tool-calling support.",
        "zh": "统一 AI LLM 处理器：支持 11 个供应商（DeepSeek、智谱、千帆、MiniMax、Anthropic、Hyperspace、豆包、月之暗面、Gemini、Ollama、OpenAI 兼容），含 MCP 工具调用能力。",
    },
