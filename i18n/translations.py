...
TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "ai_gen_welcome": {"en": "Connected to {provider} ({model}).\n\nCategory overview:\n• Selenium — Browser automation (navigate, click, input, collect)\n• HTTP — HTTP requests and HTML parsing\n• File — File read/write/delete/upload\n• Excel/CSV — Spreadsheet operations\n• Database — SQL database access\n• Email — Send/receive emails\n• Paramiko — SSH/SFTP remote operations\n• DataProcessing — Collection merge, filter, transform\n• JSON — JSON file read/write\n• Mouse/GUI — Mouse clicks, keyboard, dialogs\n• AI_LLM — LLM integration\n• General — Flow control, wait, conditions, commands\n\nTip: Select needed processors to save tokens, or select all if unsure.", "zh": "已连接 {provider} ({model})。\n\n分类概览：\n• Selenium — 浏览器自动化（导航、点击、输入、采集）\n• HTTP — HTTP 请求和 HTML 解析\n• File — 文件读写、删除、上传\n• Excel/CSV — 表格操作\n• Database — SQL 数据库访问\n• Email — 收发邮件\n• Paramiko — SSH/SFTP 远程操作\n• DataProcessing — 集合合并、过滤、转换\n• JSON — JSON 文件读写\n• Mouse/GUI — 鼠标点击、键盘、弹窗\n• AI_LLM — 大模型集成\n• General — 流程控制、等待、条件、命令\n\n提示：选中需要的 Processor 可节省 Token；不确定时可全选，让 AI 帮你选择。"},
