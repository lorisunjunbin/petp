...
class FIND_THEN_KEYINProcessor(Processor):
    TPL: str = '{"find_by":"id|xpath|css", "identity":"", "value":"", "value_key":"", "clear_before_input":"yes|no","wait":1, "timeout":5, "skip_timeout_error":"yes|no", "timeout_msg":"", "skip_if_fn":"return False", "chrome_name":"chrome"}'
    DESC = '''
    ...
    - timeout_msg: optional business message appended to the raised exception when the element is not found AND skip_timeout_error is "no". Gives the hard failure domain context. Supports expression against data_chain AND the special {timeout} variable, e.g. "Can not find the supplier {supplier_name} in {timeout} seconds." Ignored when skip_timeout_error is "yes". (default: "")
    ...
    def process(self):
        ...
        if not ele is None:
            ...
            self.fail_or_skip('element not found within %ss: %s -> %s' % (timeout, keyinby, identity),
                              False, prefix='FIND_THEN_KEYIN',
