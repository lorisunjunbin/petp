...
class SCREENSHOTProcessor(Processor):
    ...
    DESC: str = f''' 
        - Make a screenshot via selenium within chrome web page, store image location to filepath_key of data_chain.
    ...
    def process(self):
        ...
        if self.has_param("crop"):
            ...
            SeleniumUtil.screenshot_with_crop(chrome, int(arr[0]), int(arr[1]), int(arr[2]), int(arr[3]), fp, fp_org, show, format)

        # 这个方法crop的不准，少用！
        elif self.has_param('xpath'):
            SeleniumUtil.screenshot_by_x_path(chrome, self.get_param('xpath'), fp, fp)
