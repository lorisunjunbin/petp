...
| Date | What's New |
|------|------------|
| 2026-09 | 🔒 **Auth token via environment variable**: `http_request_token` and `ai_api_key` now resolve `${ENV_VAR}` references, so the real secret stays out of `petpconfig.yaml` (e.g. `http_request_token: ${PETP_HTTP_TOKEN}`). New `python tools/gen_token.py` prints a strong `secrets.token_urlsafe(32)` token; shared resolver in `utils/SecretUtil.py` |
| 2026-09 | 🔒 **OAuth2 / JWT auth (resource server)**: new `auth_mode: oauth2` validates inbound `Authorization: Bearer` JWTs locally against a JWKS endpoint (`oauth2_jwks_url`, optional `oauth2_audience` / `oauth2_issuer`) via `PyJWT` — no IdP SDK required; `auth_mode: static` (default) keeps the existing token check. Provider in `httpservice/auth/OAuth2JwtProvider.py` |
| 2026-09 | **Config comments preserved**: `petpconfig.yaml` now round-trips through `ruamel.yaml`, so bilingual comments and key order survive GUI/`set_config` writes instead of being stripped on every save |
| 2026-09 | **Removed legacy `enable_windows_hdpi`**: the flag only re-invoked `SetProcessDpiAwareness(2)` (V1) after startup already set PER_MONITOR_AWARE_V2 — a redundant call that would fail with `E_ACCESSDENIED`. Removed the flag, `OSUtils.ensure_hdpi()`, and dead `init_display()`; DPI behavior unchanged |
